"""Deploy the V2 QA fixes without publishing a client release."""
...
def data_manifest(client):
    command = (
        f"cd {quoted(posixpath.join(REMOTE_ROOT, 'data'))} && "
...
def backup_production(client, backup_dir):
    run(client, f"mkdir -p {quoted(posixpath.join(backup_dir, 'code'))}")
    ...
    run(client, f"cp -a {quoted(posixpath.join(REMOTE_ROOT, 'secrets'))} {quoted(posixpath.join(backup_dir, 'secrets'))}")
    for relative_path in FILES:
        source = posixpath.join(REMOTE_ROOT, relative_path)
        target = posixpath.join(backup_dir, "code", relative_path)
    ...
    run(client, f"tar -C {quoted(posixpath.dirname(backup_dir))} -czf {quoted(archive_path)} {quoted(posixpath.basename(backup_dir))}", timeout=1800)
    run(client, f"tar -tzf {quoted(archive_path)} >/dev/null", timeout=1800)
    return {
        "directory": backup_dir,
        "archive": archive_path,
        "archiveSha256": remote_sha256(client, archive_path),
        "postgresDumpSha256": remote_sha256(client, dump_path),
    }
...
def verify_deployment(client, expected_hashes):
    for relative_path, expected_hash in expected_hashes.items():
        deployed_hash = remote_sha256(client, posixpath.join(REMOTE_ROOT, relative_path))
    ...
    required_admin_markers = ["validateCustomRange", "XMAI_ADMIN_CONTEXT", "updateDirectFolderStatus", "x-admin-name"]
    required_platform_markers = ["trendGranularity", "seedTrendBuckets", "taxonomyErrorMessage", "normalizedFact"]
    required_company_markers = ["adminActorName", "prepareDirectPublishPackage", "idempotencyKey", "updatedAt = revokedAt"]
    if not all(marker in admin_js for marker in required_admin_markers):
        raise RuntimeError("deployed administrator UI is missing QA fixes")
    if not all(marker in platform_source for marker in required_platform_markers):
        raise RuntimeError("deployed V2 platform service is missing QA fixes")
    if not all(marker in company_source for marker in required_company_markers):
        raise RuntimeError("deployed company-skill service is missing QA fixes")
...
def main():
    ...
    backup_dir = posixpath.join(REMOTE_ROOT, "backups", f"v2-qa-fixes-{timestamp}")
    uploaded = []
    ...
    try:
        ...
        for relative_path in FILES:
            ...
            if not os.path.isfile(local_path):
                raise RuntimeError(f"deployment file is missing: {relative_path}")
            ...
            if remote_sha256(client, temporary_path) != expected_hash:
        ...
        print(json.dumps({
            ...
            "dataVerifiedBeforeRestart": True,
            "files": FILES,
        }, ensure_ascii=False))
    except Exception:
